...
module.exports = function (content) {
  ...
  var isDirectory = options.directory && filePath.endsWith('index.js')
  var parserFn
  ...
  if (isDirectory) {
    ...
    parserFn = function () {
      return directory(content, filePath, loaders, loaderContext, function (asyncParts) {
        loaderCallback(null, generateOutput(asyncParts))
      })
    }
  } else {
    parserFn = function () {
      return parse(content, fileName, this.sourceMap, loaderContext)
    }
  }

  // Synchronously return result if not loading a directory
  var parseResult = parserFn()
  if (parseResult) {
    return generateOutput(parseResult)
  }
}
