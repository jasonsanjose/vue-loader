...
module.exports = function (content) {
  ...
  function doParse () {
    var parserFn = options.directory ? directory : parse
    return parserFn(content, fileName, this.sourceMap, loaderContext, parseCallback)
  }

  if (options.directory) {
    var loaderCallback = this.async()
    parseCallback = function (asyncParts) {
      loaderCallback(null, generateOutput(asyncParts))
    }
  }

  // Synchronously return result if not loading a directory
  var parseResult = doParse()
  if (parseResult) {
